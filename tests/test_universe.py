...
class UniverseTests(unittest.TestCase):
    ...
    def test_owner_map_source_preview_save_and_history(self):
        ...
        with patch('requests.get', side_effect=get), patch('requests.post', side_effect=post):
            ...
            by_label(app.checkbox,'Include company-hosted PDF releases').set_value(True)
            by_label(app.text_input,'Article array path').set_value('props.pageProps.news')
            ...
            self.assertTrue(posted[-1][1]['operation']['value']['allow_pdf'])
            self.assertEqual(posted[-1][1]['operation']['value']['json_items_path'],'props.pageProps.news')
            self.assertEqual(posted[-2][1]['json_items_path'],'props.pageProps.news')
