...
def rejected_flag_popover(items):
    item_map = {
        int(item["id"]): item
        for item in items
    ...
    with st.popover("Flag a rejected item"):
        st.caption("Choose an item from this page. The event ID is filled automatically.")
        selected_id = st.selectbox(
            ...
            key="rejected_flag_id",
        )
        verdict = st.radio(
            "Verdict",
            ["👍 should have ranked", "👎 correct rejection"],
            horizontal=True,
            key="rejected_flag_verdict",
        )
        note = st.text_area(
            "Note (optional)",
            key="rejected_flag_note",
            height=80,
            placeholder="e.g. this major partnership should have ranked",
        )

        if st.button("Submit flag", type="primary", key="rejected_flag_submit"):
            pin = str(st.session_state.get("grader_pin", "")).strip()
            if not pin:
                st.warning("Enter the grader PIN in Owner mode first.")
                return
            try:
                ...
                handle_grade_response(
                    response,
                    "Flag stored — the next digest run will absorb the feedback.",
                )
            except Exception as exc:
                st.error(f"Could not reach the aggregator: {exc}")
...
with owner_col:
    with st.popover("Owner mode"):
        ...
        st.caption(
            "Use Rate digest or Flag a rejected item to turn feedback into ranking precedents."
        )
...
with tab_rejected:
    ...
    if rejected:
        ...
        rejected_flag_popover(page_items)
